...
load("PRE_items.Rda")
pr_i
#load("rel_fit.Rda") ## should not standardize
#plot(rel_fit) ## should not standardize
load("rel_fit2.Rda")
plot(rel_fit2)
pr_i2 <- pr_i
pr_i2$glmnet.fit <- rel_fit2
pr_i2

## Compare accuracy
pr_preds_train_i <- predict(pr_i, type = "response")
pr_preds_test_i <- predict(pr_i, newdata = data[test_ids , ], type = "response")
mean((train_y - pr_preds_train_i)^2)
mean((test_y - pr_preds_test_i)^2)

pr_preds_train_i2 <- predict(pr_i2, type = "response", 
                             penalty.par.val = "lambda.1se", newdata = data[train_ids , ])
pr_preds_test_i2 <- predict(pr_i2, newdata = data[test_ids , ], type = "response",
                            penalty.par.val = "lambda.1se",)
mean((train_y - pr_preds_train_i2)^2)
mean((test_y - pr_preds_test_i2)^2)
##undebug(pre:::predict.pre)
## If gamma is not specified to predict.cv.glmnet, it is used anyway when s = "lambda.1se" 
```

What if we want only 50 rules?

```{r}
pr_i2$glmnet.fit$relaxed$statlist$`g:0`$nzero
pr_i2$glmnet.fit$relaxed$statlist$`g:0.25`$nzero
pr_i2$glmnet.fit$relaxed$statlist$`g:0.5`$nzero
pr_i2$glmnet.fit$relaxed$statlist$`g:0.75`$nzero
pr_i2$glmnet.fit$relaxed$statlist$`g:1`$nzero
## Then use s21, all have same number of variables
## And the same lambda value

## gamma = 0 is best: makes sense, is forward stagewise
lambda <- pr_i2$glmnet.fit$relaxed$statlist$`g:1`$lambda[21]

pr_preds_train_i3 <- predict(pr_i2, type = "response", gamma = 0, 
                             penalty.par.val = lambda, newdata = data[train_ids , ])
pr_preds_test_i3 <- predict(pr_i2, newdata = data[test_ids , ], type = "response",
                            penalty.par.val = lambda, gamma = 0)
mean((train_y - pr_preds_train_i3)^2)
mean((test_y - pr_preds_test_i3)^2)


pr_preds_train_i4 <- predict(pr_i2, type = "response", gamma = 1, 
                             penalty.par.val = lambda, newdata = data[train_ids , ])
pr_preds_test_i4 <- predict(pr_i2, newdata = data[test_ids , ], type = "response",
                            penalty.par.val = lambda, gamma = 1)
mean((train_y - pr_preds_train_i4)^2)
mean((test_y - pr_preds_test_i4)^2)


summary(pr_i2, gamma = 0, penalty.par = lambda)
imps <- pre:::importance(pr_i2, gamma = 0, penalty.par = lambda)
imps$varimps
```
...
preds_train <- data.frame(bm_preds_train = rep(mean(train_y), times = length(train_ids)), 
                          ...
                          ct_preds_train_i, pr_preds_train_i3, gb_preds_train_i,
                          rf_preds_train_i, knn_preds_train_i)
preds_test <- data.frame(bm_preds_test = rep(mean(train_y), times = length(test_ids)),
                          glm_preds_test_i, gam_preds_test_i,
                          ct_preds_test_i, pr_preds_test_i3, gb_preds_test_i, 
